...
public class NetGHIJEF extends AutoModeBase {
	public NetGHIJEF(AutoFactory factory) {
		super(factory, "Net GH IJ EF");
		...
		Pose2d endingPose = new Pose2d(
				thirdAlignPose.getMeasureX().plus(Units.Inches.of(RobotConstants.isRedAlliance ? -3.0 : 3.0)),
		...
		AutoTrajectory ijAlgaeToNet = trajectory("ijAlgaeToRightNet");
		AutoTrajectory netToEFAlgae = trajectory("IJNetToEFAlgae");
		AutoTrajectory efAlgaeToNetEarlyStop = trajectory("efAlgaeToNet", 0);
		AutoTrajectory efAlgaeToNetContinue = trajectory("efAlgaeToNet", 1);
		...
		prepRoutine(
				...
				s.reefAlgaeIntakeInAuto(true)
						.asProxy()
						.deadlineFor(netToEFAlgae.cmd().andThen(s.goToReefIntakePose(Face.FAR_RIGHT))),
				Commands.either(
						Commands.parallel(
								Commands.waitSeconds(1.0)
										.andThen(s.processorPrep().asProxy()),
								efAlgaeToNetEarlyStop
										.cmd()
										.andThen(new PIDToPoseCommand(efAlgaeToNetEarlyStop
												.getFinalPose()
												.get()))),
						Commands.sequence(
								efAlgaeToNetEarlyStop
										.cmd()
										.andThen(efAlgaeToNetContinue.cmd())
										.deadlineFor(Commands.waitSeconds(0.8)
						...
						() -> Robot.autoTimer.getTime().gte(Units.Seconds.of(15.0))));
	}
}
