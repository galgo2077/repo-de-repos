...
public class NetGHEFIJ extends AutoModeBase {
	public NetGHEFIJ(AutoFactory factory) {
		super(factory, "Net GH EF IJ");
		...
		AutoTrajectory netToEFAlgae = trajectory("GHNetToEFAlgae");
		AutoTrajectory efAlgaeToNet = trajectory("efAlgaeToNet");
		AutoTrajectory netToIJAlgae = trajectory("netToIJAlgae");
		AutoTrajectory ijAlgaeToNetEarlyStop = trajectory("ijAlgaeToRightNet", 0);
		AutoTrajectory ijAlgaeToNetContinue = trajectory("ijAlgaeToRightNet", 1);
		...
		prepRoutine(
				...
				Commands.either(
						Commands.parallel(
								...
								ijAlgaeToNetEarlyStop
										.cmd()
										.andThen(new PIDToPoseCommand(ijAlgaeToNetEarlyStop
												.getFinalPose()
												.get()))),
						Commands.sequence(
								ijAlgaeToNetEarlyStop
										.cmd()
										.andThen(ijAlgaeToNetContinue.cmd())
										.deadlineFor(Commands.waitSeconds(0.5)
